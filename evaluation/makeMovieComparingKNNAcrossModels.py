...
from utils import ALL_KNN_MOVIE_TEST_SETS, BENCHMARK_DATASETS, FOLDER_NAME_FOR_KNN_GIF_SEQS, PATH_TO_MOSAICS
from utils import get_data_folder_from_model_name, get_movie_test_set_for_data_folder
from utils import get_immediate_subdirectories_path, get_immediate_files_in_path, create_mosaic_img_and_save
...
for data_folder, test_set in zip(datasets, ALL_KNN_MOVIE_TEST_SETS):
    ...
    for model_folder in models_for_a_dataset:
        ...
        if 'Supervised' in model_folder or 'supervised' in model_folder:
            ...
            if model_folder != '' and len(
                    model_folder) > 0:  # # TODO elif 'Priors' in model_folder: #                print 'Processi           print 'Processing Robotics priors model: ', path_to_neighbors
                priors_imgs = get_immediate_files_in_path(path_to_neighbors,
    ...
    if len(test_set) > 0 and len(AE_imgs) > 0 and len(supervised_imgs) > 0 and len(priors_imgs) > 0:
        ...
        if use_ground_truth:
            if use_ground_truth and len(GT_imgs) == 0:
                sys.exit(
                    'If you want to also plot ground truth neighbors, you should run first generate_neighbors_for_all_models_movie.sh and include a GT folder model there')
            for input_img_test, gt, superv, prior, ae in zip(test_set, GT_imgs, supervised_imgs, priors_imgs, AE_imgs):
                ...
                create_mosaic_img_and_save(input_img_test, [gt, superv, prior, ae], path_to_mosaic_images,
                                           'mosaic_' + str(index) + '.jpg',
                                           top_title=get_data_folder_from_model_name(input_img_test),
                                           titles=['Ground Truth', 'Supervised (Robot Hand Position)', 'Robotic Priors',
                                                   'Denoising Auto-Encoder'])
                index += 1
        else:
            for input_img_test, superv, prior, ae in zip(test_set, supervised_imgs, priors_imgs, AE_imgs):
                path_to_mosaic_images = PATH_TO_MOSAICS + data_folder + '/'
                create_mosaic_img_and_save(input_img_test, [superv, prior, ae], path_to_mosaic_images,
                                           'mosaic_' + str(index) + '.jpg',
                                           top_title=get_data_folder_from_model_name(input_img_test),
                                           titles=['Supervised (Robot Hand Position)', 'Robotic Priors',
                                                   'Denoising Auto-Encoder'])
                index += 1
