...
# coding: utf-8
# from utils import BABBLING, MOBILE_ROBOT, SIMPLEDATA3D, PUSHING_BUTTON_AUGMENTED,
# STATIC_BUTTON_SIMPLEST,LEARNED_REPRESENTATIONS_FILE
...
header = ['Model', 'KNN_MSE', 'DATA_FOLDER', 'MODEL_ARCHITECTURE_FILE', 'MAX_COS_DIST_AMONG_ACTIONS_THRESHOLD',
          'CONTINUOUS_ACTION_SIGMA', 'STATES_DIMENSION']
...
if os.path.isfile(GLOBAL_SCORE_LOG_FILE):
    if os.path.isfile(MODELS_CONFIG_LOG_FILE):
        mse_df = pd.read_csv(GLOBAL_SCORE_LOG_FILE,
                             usecols=['Model', 'KNN_MSE', 'STATES_DIMENSION'])  # columns = header)
        models_df = pd.read_csv(MODELS_CONFIG_LOG_FILE, usecols=['Model', 'DATA_FOLDER', 'MODEL_ARCHITECTURE_FILE',
                                                                 'MAX_COS_DIST_AMONG_ACTIONS_THRESHOLD',
                                                                 'CONTINUOUS_ACTION_SIGMA'])  # , columns = header)   print mse_df.head()    print models_df.head()
        ...
        all_scores_logs = mse_df.fillna(np.nan).dropna().merge(models_df.fillna(np.nan).dropna(), on='Model',
                                                               how='outer')
